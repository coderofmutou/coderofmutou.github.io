...
MySQL 提供了丰富的内置函数，这些函数使得数据的维护与管理更加方便，能够更好地提供数据的分析与统计功能。

MySQL 内置函数从功能角度可分为数值函数、字符串函数、日期和时间函数、流程控制函数、加密与解密函数、获取 MySQL 信息函数、聚合函数等。本章及下一章主要讲解单行函数与聚合函数。
...
**与 ATAN(X) 的区别**：ATAN2 需要两个参数 `point(x1,y1)` 和 `point(x2,y2)`,其计算公式为 `ATAN2(y2-y1, x2-x1)`。当 `x2-x1` 等于 0 时,ATAN(X) 会报错,而 ATAN2(M,N) 仍可正常计算。
...
![image-20211025214818623](images/image-20211025214818623.webp)
...
![image-20211012143203355](images/image-20211012143203355.webp)
...
![image-20211012145231321](images/image-20211012145231321.webp)
...
![1554980865631](images/1554980865631.webp)
