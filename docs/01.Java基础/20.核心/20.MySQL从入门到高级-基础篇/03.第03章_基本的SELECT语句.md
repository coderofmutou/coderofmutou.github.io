...
::: note
因为查询语句使用非常频繁，所以很多人把查询语句单拎出来一类：DQL（数据查询语言）。
...
::: tip
MySQL 中的 SQL 语句是不区分大小写的，因此 SELECT 和 select 的作用是相同的，但是，许多开发人员习惯将关键字大写、数据列和表名小写，建议养成统一的书写规范，这样写出来的代码更容易阅读和维护。
:::
