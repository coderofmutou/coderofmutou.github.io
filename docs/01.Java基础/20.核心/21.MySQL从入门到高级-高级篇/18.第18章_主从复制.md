...
![Master_Slave_Replication2.jpg](./images/Master_Slave_Replication2.jpg)
...
在 Mixed 模式下，一般的语句修改使用 statment 格式保存 binlog。如一些函数，statement 无法完成主从 复制的操作，则采用 row 格式保存 binlog。 
...
>
>第二次 change master 就不要加 MASTER_SSL=1;了
...
![image-20241202222315787.jpg](./images/image-20241202222315787.jpg)
...
在 MySQL5.7 版本中还增加了一个`rpl_semi_sync_master_wait_for_slave_count`参数，可以对应答的 从库数量进行设置，默认为`1`，也就是说只要有 1 个从库进行了响应，就可以返回给客户端。如果将这个参数调大，可以提升数据一致性的强度，但也会增加主库等待从库响应的时间。【以时间换取一致性】
...
![image-20241202215856411.jpg](./images/image-20241202215856411.jpg)

![image-20241202215927317.jpg](./images/image-20241202215927317.jpg)
