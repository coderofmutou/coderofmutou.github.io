...
![image-20241209004210746](./images/image-20241209004210746.webp)
...
1. 取最近一次 `全量备份`。假设设置数据库备份是一天一备，最近备份数据是当天 `凌晨 2 点`；
...
对于一个有高可用机制的 MySQL 集群来说，不用担心 `rm 删除数据`。因为只删除掉其中某一个节点数据的话，HA 系统就会选出一个新的主库，从而保证整个集群的正常工作。我们把这个节点上的数据恢复回来后，再接入整个集群就好了。
