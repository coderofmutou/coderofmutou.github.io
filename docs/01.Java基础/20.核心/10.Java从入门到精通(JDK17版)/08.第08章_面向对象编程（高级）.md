...
![第2阶段：Java面向对象编程-第08章](images/第2阶段：Java面向对象编程-第08章.webp)
...
![image-20220104100145059](images/image-20220104100145059.webp)
...
::: note 💡 背景
对软件设计模式的研究造就了一本可能是面向对象设计方面最有影响的书籍：《Design Patterns: Elements of Reusable Object-Oriented Software》（即《设计模式》），由 Erich Gamma、Richard Helm、Ralph Johnson 和 John Vlissides 合著（Addison-Wesley，1995）。这几位作者常被称为"四人组（Gang of Four）"，这本书也被称为"GoF 书"。
:::
...
如果我们要让类在一个虚拟机中只能产生一个对象，我们首先必须将类的构造器的访问权限设置为 private，这样，就不能用 new 操作符在类的外部产生类的对象了，但在类内部仍可以产生该类的对象。因为在类的外部开始还无法得到类的对象，`只能调用该类的某个静态方法`以返回类内部创建的对象，静态方法只能访问类中的静态成员变量，所以，指向类内部产生的该类对象的变量也必须定义成静态的。
...
![image-20211228102022216](images/image-20211228102022216.webp)
...
![image-20220517205013997](images/image-20220517205013997.webp)
...
![bbcc80f541000c71b81650cfaa770c86](images/bbcc80f541000c71b81650cfaa770c86.webp)
...
![image-20220514163212312](images/image-20220514163212312.webp)
...
![image-20220325235321778](images/image-20220325235321778.webp)
...
![image-20220328002053452](images/image-20220328002053452.webp)
...
![image-20220504172918861](images/image-20220504172918861.webp)
...
::: note 拓展：元数据
元数据（Metadata）是描述数据的数据。例如：`String name = "Tom";` 中，`name` 是对 `"Tom"` 进行说明的元数据。
:::
...
![image-20220511181800694](images/image-20220511181800694.webp)

![image-20220524102038600](images/image-20220524102038600.webp)
...
![image-20211228180938922](images/image-20211228180938922.webp)

![image-20221002195547325](images/image-20221002195547325.webp)

其中，junit-libs 包内容如下：

![image-20220813005206452](images/image-20220813005206452.webp)

第 2 步：选择要在哪些 module 中应用 JUnit 库

![image-20220813005511062](images/image-20220813005511062.webp)

第 3 步：检查是否应用成功

![image-20220813005729233](images/image-20220813005729233.webp)

**注意 Scope：选择 Compile，否则编译时，无法使用 JUnit。**

第 4 步：下次如果有新的模块要使用该 libs 库，这样操作即可

![image-20220813005944022](images/image-20220813005944022.webp)

![image-20220813010018152](images/image-20220813010018152.webp)

![image-20220813010055217](images/image-20220813010055217.webp)

![image-20220813010124381](images/image-20220813010124381.webp)
...
![image-20220106152412245](images/image-20220106152412245.webp)
...
![image-20220813011625546](images/image-20220813011625546.webp)

![image-20220813011642180](images/image-20220813011642180.webp)
...
![image-20211229100040505](images/image-20211229100040505.webp)
...
![image-20220329001912486](images/image-20220329001912486.webp)
...
![image-20220514163725830](images/image-20220514163725830.webp)
...
::: note 💡 个人注
转为基本数据类型，一般是因为需要运算，Java 中的大多数运算符是为基本数据类型设计的，如比较、算术等。
:::
...
![image-20220813012801907](images/image-20220813012801907.webp)
