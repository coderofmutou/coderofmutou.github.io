...
- 下载网址：[IntelliJ IDEA 下载页](https://www.jetbrains.com/idea/download/#section=windows)
...
两个不同版本的详细对比，可以参照[官网版本对比页](https://www.jetbrains.com/idea/features/editions_comparison_matrix.html)。

官网提供的详细使用文档：[Meet IntelliJ IDEA](https://www.jetbrains.com/help/idea/meet-intellij-idea.html)
...
- 选择 3：学生和教职人员可通过 [JetBrains 教育授权计划](https://www.jetbrains.com/student/) 申请免费使用。
...
打开 `%APPDATA%\JetBrains\IntelliJIdea2022.1\idea64.exe.vmoptions` 文件。
...
原因：安装新版 IDEA 时，会自动将旧版本的启动参数（`idea64.exe.vmoptions`）复制过来。若旧版本含有当前新版不兼容的 JVM 参数，会导致 JVM 启动失败。删除红框中的参数后即可恢复正常启动。
...
（快捷键查看方式：菜单 **Help → Keymap Reference**，或访问 [IntelliJ IDEA 默认快捷键参考](https://www.jetbrains.com/help/idea/reference-keymap-win-default.html)）
...
::: tip 经验
初学者对于在哪里加断点，缺乏经验，这也是调试程序最麻烦的地方，需要一定的经验。
