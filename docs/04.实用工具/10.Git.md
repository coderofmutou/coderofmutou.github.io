...
> 典型场景：正在开发功能 A，临时需要切到其他分支修 bug，先 `git stash` 保存现场，修完再 `git stash pop` 恢复。
...
> ⚠️ `.gitignore` 只对**尚未被 Git 追踪**的文件生效。如果文件已被提交过，需先执行 `git rm --cached <file>` 从索引中移除，再让 `.gitignore` 生效。
