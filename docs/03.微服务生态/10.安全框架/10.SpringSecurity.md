...
>
> —— 引自 [Spring Security 官网](https://spring.io/projects/spring-security)
...
3. **Web**
   ...
   `spring-security-web.jar` 包含过滤器和相关的 Web 安全基础结构代码。任何具有 Servlet API 依赖性的东西。如果需要 Spring Security web 身份验证服务和基于 URL 的访问控制，则需要它。主要包是 `org.springframework.security.web`。

4. **Config**

   `spring-security-config.jar` 包含安全名称空间解析代码和 Java 配置代码。如果使用 Spring Security XML 命名空间进行配置或 Spring Security 的 Java 配置支持，则需要它。主要包是 `org.springframework.security.config`。这些类都不打算直接用于应用程序。
...
8. **OAuth 2.0 JOSE**
   ...
   `spring-security-oauth2-jose.jar` 包含 Spring Security 对 JOSE（JavaScript 对象签名和加密）框架的支持。JOSE 旨在提供安全地传输双方之间的权利要求的方法。它由以下规范构建：
   ...
   包含顶级包：`org.springframework.security.oauth2.jwt`、`org.springframework.security.oauth2.jose`。
...
10. **CAS**
    ...
    `spring-security-cas.jar` Spring Security CAS 客户端集成。如果要对 CAS 单点登录服务器使用 Spring Security web 身份验证，则需要它。顶级包是 `org.springframework.security.cas`。

11. **OpenID**

    `spring-security-openid.jar` OpenID web 身份验证支持。用于针对外部 OpenID 服务器对用户进行身份验证。顶级包是 `org.springframework.security.openid`，需要 OpenID4Java。
...
```
#### 配置类实现 WebSecurityConfigurerAdapter 类（Spring Security 5.x）

::: warning 版本说明
`WebSecurityConfigurerAdapter` 自 **Spring Security 5.7**（Spring Boot 2.7）起被标记为 deprecated，并于 **Spring Security 6.0** 中彻底移除。Spring Boot 3.x 项目请使用 `SecurityFilterChain` Bean 方式，见"Spring Security 快速入门"章节。
:::
...
我们也可以将用户名、密码配置在 Spring Boot 的配置文件中：在 `application.properties` 中配置自定义用户名和密码
...
将访问令牌包含在 URL 锚点中的好处：锚点在 HTTP 请求中不会发送到服务器，减少了泄漏令牌的风险。
```
...
::: warning 版本说明
本节内容适用于 **Spring Security 5.x（Spring Boot 2.x）**，使用 `WebSecurityConfigurerAdapter` 继承方式进行配置。Spring Security 6.0（Spring Boot 3.x）已废弃该方式，请参考前面章节中的 `SecurityFilterChain` 配置方式。
:::
...
::: note
Spring Security 在默认的认证过程中如果账号密码校验成功会返回 Authentication 对象，之后 `UsernamePasswordAuthenticationFilter` 会将用户信息 `Authentication` 存入 `SecurityContextHolder` 中。但实际场景中认证通过后还需要向前端返回包含 JWT 的 JSON 数据，所以需要写一个自定义登录接口。
:::
...
::: note
Spring Security 默认是在内存中查找对应的用户名密码，封装成 `UserDetails` 对象后交给 `DaoAuthenticationProvider` 校验。实际场景中需要从数据库查找用户信息，因此需要实现 `UserDetailsService` 接口。
:::
