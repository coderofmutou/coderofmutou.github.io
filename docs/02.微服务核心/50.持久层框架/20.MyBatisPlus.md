---
title: 持久层框架之 MyBatis-Plus
date: 2024-07-11 20:54:23
permalink: /micro-service-core/mybatis-plus/
categories: 
  - 微服务核心
  - 持久层框架
tags:
...
::: note
IDEA 在 `userMapper` 处报错，因为找不到注入的对象（类是动态创建的），但程序可以正确执行。为了避免报错，可以在 mapper 接口上添加 `@Repository` 注解。
:::
...
::: note IService 说明
- `IService` 是 MyBatis-Plus 提供的通用 Service 层接口，封装了常见的 CRUD 操作（插入、删除、查询、分页等）
- 方法命名规范：`get` 查询单行，`remove` 删除，`list` 查询集合，`page` 分页查询
- 泛型 `T` 为任意实体对象；`Wrapper` 为条件构造器
- 建议：如需自定义通用 Service 方法，创建自己的 `IBaseService` 继承 `IService`
