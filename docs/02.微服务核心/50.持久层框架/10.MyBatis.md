...
核心配置文件主要用于配置连接数据库的环境以及 MyBatis 的全局配置信息，存放在 `src/main/resources` 目录下。
:::
...
::: note
MyBatis 中的 mapper 接口相当于以前的 dao，但区别在于 mapper 仅仅是接口，不需要提供实现类。
:::
...
::: note 映射文件规范
1. 映射文件命名规则：表对应实体类的类名 + `Mapper.xml`，例如表 `t_user` → 实体类 `User` → 映射文件 `UserMapper.xml`
   - 一个映射文件对应一个实体类，对应一张表的操作
   - MyBatis 映射文件用于编写 SQL，存放于 `src/main/resources/mappers` 目录下
2. MyBatis 中面向接口操作数据，需保证两个一致：
   - mapper 接口的全类名与映射文件的命名空间（`namespace`）一致
   - mapper 接口中方法名与映射文件中 SQL 标签的 `id` 属性一致
:::
...
::: note 日志级别
FATAL（致命）> ERROR（错误）> WARN（警告）> INFO（信息）> DEBUG（调试），从左到右打印内容越来越详细。
:::
...
::: note
最新版 IDEA 已自带映射文件模板，且 MyBatisX 插件也可快捷创建映射文件。
:::
...
::: tip 字段与属性名不一致时的两种处理方式
若字段名符合数据库规则（`_` 下划线），实体类属性名符合 Java 规则（驼峰），可通过以下两种方式处理映射关系：

1. 为字段起别名，保证与实体类属性名一致：

   ```sql
   select eid, emp_name empName, age, sex, email from t_emp
   ```

2. 在 MyBatis 核心配置文件中设置全局配置 `mapUnderscoreToCamelCase`，查询时自动将 `_` 类型字段名转为驼峰：
...
::: tip 分步查询的优点：延迟加载
分步查询可以实现延迟加载，但须在核心配置文件中设置：

- `lazyLoadingEnabled`：延迟加载全局开关，默认 `false`，需手动开启：

  ```xml
  <setting name="lazyLoadingEnabled" value="true"/>
  ```

- `aggressiveLazyLoading`：开启时任何方法调用都会加载该对象所有属性，默认 `false`（按需加载）

开启后，可通过 `association` 和 `collection` 的 `fetchType` 属性手动控制单个分步查询是否延迟加载：`fetchType="eager"` 立即加载，`fetchType="lazy"` 延迟加载。
:::
...
::: note `where` 与 `if` 结合使用
1. 若 `where` 标签中的 `if` 条件都不满足，则 `where` 标签不会添加 `where` 关键字
2. 若有条件满足，`where` 标签会自动添加 `where` 关键字，并去掉条件最前方多余的 `and/or`
3. 注意：`where` 标签不能去掉条件**最后**多余的 `and/or`
:::
...
::: note `trim` 常用属性
- `prefix`：在内容前面添加指定内容
- `prefixOverrides`：去掉内容前面的指定内容
- `suffix`：在内容后面添加指定内容
- `suffixOverrides`：去掉内容后面的指定内容
:::
...
PageInfo{
  ...
  prePage=7, nextPage=0, isFirstPage=false, isLastPage=true, hasPreviousPage=true, hasNextPage=false,
